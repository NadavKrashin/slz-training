...
export function UserDetailClient() {
  ...
  useEffect(() => {
    const unsub = subscribeToUser(uid, (data) => { setProfile(data); setLoading(false); });
    return unsub;
  }, [uid]);
  ...
  if (loading) return <PageContainer><Text c="dimmed">טוען...</Text></PageContainer>;

  return (
    <PageContainer>
      <Stack gap="lg">
        <Group>
          <ActionIcon variant="subtle" aria-label="חזרה לרשימת משתמשים" onClick={() => router.push('/admin/users')}><IconChevronRight size={20} /></ActionIcon>
          <Text size="xl" fw={700}>{profile?.displayName}</Text>
        </Group>
        <Card>
          <Stack gap="xs">
            ...
            <Group>
              <Text size="sm">שיתוף: <Text component="span" fw={700}>{profile?.shareCompletionWithAdmin ? 'כן' : 'לא'}</Text></Text>
            </Group>
          </Stack>
        </Card>
        ...
        {compLoading ? (
          ...
          <>
            ...
            <MonthlySummary completed={completed} missed={total - completed} total={total} streak={0} />
          </>
        )}
      </Stack>
    </PageContainer>
