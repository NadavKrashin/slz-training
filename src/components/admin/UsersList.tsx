  ...
  return (
    <Stack gap="md">
      <Group justify="space-between">
        ...
        {guestCount > 0 && (
          <Switch
            label={`הצג אורחים (${guestCount})`}
            checked={showGuests}
            onChange={(e) => setShowGuests(e.currentTarget.checked)}
          />
        )}
      </Group>
          ...
          {filtered.map((u) => (
            ...
            >
              ...
              <Table.Td>
                <Badge
                  variant="light"
                  color={u.role === 'admin' ? 'brand' : u.role === 'guest' ? 'yellow' : 'gray'}
                >
                  {u.role === 'admin' ? 'מנהל' : u.role === 'guest' ? 'אורח' : 'משתמש'}
                </Badge>
              </Table.Td>
              <Table.Td>
                <Group gap={4}>
