...
export function StageDisplay({
  ...
  return (
    <Stack align="center" gap="sm">
      <Text size="sm" c={inverted ? 'rgba(255,255,255,0.7)' : 'dimmed'}>
      ...
      <Group gap="sm" align="center">
        {isRest ? (
          <IconZzz
            size={28}
            color={inverted ? 'rgba(255,255,255,0.9)' : 'var(--mantine-color-teal-6)'}
          />
        ) : (
          <IconBarbell size={28} color={inverted ? 'white' : 'var(--mantine-color-brand-6)'} />
        )}
        <Text size="1.75rem" fw={700} c={inverted ? 'white' : undefined}>
          {stage.name}
        </Text>
      </Group>
      <Badge
